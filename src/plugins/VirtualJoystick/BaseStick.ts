 ...
 */
export default class BaseStick extends EventEmitter {
	scene: Phaser.Scene;
	position: Phaser.Math.Vector2;
	line: Phaser.Geom.Line;
	baseHitArea: Phaser.Geom.Circle;
	stickHitArea: Phaser.Geom.Circle;
	baseSprite: Phaser.GameObjects.Sprite | null;
	stickSprite: Phaser.GameObjects.Sprite | null;
	limitPoint: Phaser.Math.Vector2;
	 ...
	 */
	checkDown(pointer: Phaser.Input.Pointer): void {
	 ...
	 * @param {Phaser.Input.Pointer} pointer - The Phaser Pointer that triggered the event.
	 */
	checkUp(pointer: Phaser.Input.Pointer): void {
	 ...
	 * @param {Phaser.Input.Pointer} pointer - The Phaser Pointer that triggered the event.
	 */
	moveStick(pointer: Phaser.Input.Pointer): void {
		const x = pointer.worldX;
		const y = pointer.worldY;
	 ...
	 */
	alignBottomLeft(spacing: number = 0): this {
		if (this.baseSprite) {
	 ...
	 * @return {this} This joystick instance.
	 */
	alignBottomRight(spacing: number = 0): this {
		if (this.baseSprite) {
			const w = this.baseSprite.displayWidth / 2 + spacing;
			const h = this.baseSprite.displayHeight / 2 + spacing;
	...
	destroy(): void {
		...
		this.stickHitArea = null as any;
		this.baseHitArea = null as any;
		this.line = null as any;
		this.limitPoint = null as any;
		this.pointer = null;
	 ...
	 * @return {this} This joystick instance.
	 */
	setMotionLock(value: number = 0): this {
	 ...
	 * @return {this} This joystick instance.
	 */
	setDeadZone(value: number = 0): this {
		this.deadZone = value;
	 ...
	 */
	debug(graphics?: Phaser.GameObjects.Graphics, text?: Phaser.GameObjects.Text): void {
		if (graphics) {
			graphics.clear();
	...
	get filterY(): number | string {
		if (this.y === 0) {
			...
			if (this.y < 0) {
				return 1 - parseFloat((0.5 - fy).toFixed(2));
			} else {
				return 1 - parseFloat((0.5 + fy).toFixed(2));
			}
		}
