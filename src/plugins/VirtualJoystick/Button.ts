...
export default class Button extends EventEmitter {
	 ...
	 * @return {Button} This button instance.
	 */
	setRepeatRate(rate: number = 0): this {
	...
	set posX(x: number) {
		this.sprite.x = x;
		(this.hitArea as any).x = this._shape === CONST.CIRC_BUTTON ? x : x - this.sprite.width / 2;
	}
	...
	set posY(y: number) {
		this.sprite.y = y;
		(this.hitArea as any).y = this._shape === CONST.CIRC_BUTTON ? y : y - this.sprite.height / 2;
	}
	...
	set scale(value: number) {
		...
		if (this._shape === CONST.CIRC_BUTTON) {
			(this.hitArea as Phaser.Geom.Circle).setTo(this.sprite.x, this.sprite.y, this.sprite.width);
		} else {
			(this.hitArea as Phaser.Geom.Rectangle).setTo(
				this.sprite.x,
				this.sprite.y,
				this.sprite.width,
				this.sprite.height
